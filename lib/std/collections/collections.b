...
pub fn count<T: Eq>(values: List<T>, needle: T) -> int {
    var total: int = 0
...
pub fn filter<T: Clone>(values: List<T>, keep: fn(T) -> bool) -> List<T> {
    var result: List<T> = []
    result.reserve(values.len())
    for value: T in values {
...
pub fn transform<T: Clone, U>(values: List<T>, apply: fn(T) -> U) -> List<U> {
    var result: List<U> = []
    result.reserve(values.len())
    for value: T in values {
...
pub fn unique_of<T: Eq + Hash + Clone>(values: List<T>) -> List<T> {
    var seen: Map<T, bool> = {}
    var result: List<T> = []
    seen.reserve(values.len())
    result.reserve(values.len())
...
pub fn increment<K: Eq + Hash>(inout values: Map<K, int>, key: K, delta: int) -> int {
...
pub fn get_or_insert<K: Eq + Hash, V: Clone>(
...
pub fn merge_with<K: Eq + Hash + Clone, V: Clone>(
...
pub fn remove_if<K: Eq + Hash + Clone, V: Clone>(
...
pub fn map_values<K: Eq + Hash + Clone, V: Clone, U>(
